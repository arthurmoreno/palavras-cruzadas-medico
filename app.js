(() => {
  ...
  function newGame() {
    ...
    if (entries.length === 0) {
      alert('Banco insuficiente para esta combinação. Tente outra disciplina/dificuldade.');
      return;
    }
    engine.reset(13, 13);
  ...
  function checkCompletion() {
    // completed if all letters match
    ...
    stopTimer();
    showReview();
    // simple medals/stats update in localStorage
  ...
  function showReview() {
    ...
    // Save best time for cronometrado
  ...
  canvas.addEventListener('click', (e) => {
    ...
    if (!cell) return;
    // Toggle direction when re-clicking same cell
  ...
  tabs.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.tab === 'editor') { editor.panel.hidden = false; } else { editor.panel.hidden = true; }
    });
  });
